...
class EnhancedTableAgent4Rate extends React.Component {
  constructor(props) {
    super(props)
    this.state = {
      AccountValidation: '',
            ...
            <TableBody>
              {this.props.draweeBank4RateList && this.props.draweeBank4RateList['data'].map((row, index) => {
                ...
                return (
                  ...
                  >
                    <TableCell style={{ paddingLeft: 40 }}>{row.agentName}</TableCell>
                    <TableCell>{row.agentBranchName}</TableCell>
                    {/* <TableCell>{(row.agentBranchName == null) ? 'ALL':[ (row.agentBranchName.length > 0) ? [(row.agentBranchName.length == 1) ? (row.agentBranchName[0].branchName): (row.agentBranchName[0].branchName+' ...')]: 'ALL']}</TableCell>    */}
                    <TableCell>
...
EnhancedTableAgent4Rate.propTypes = {
  classes: PropTypes.object.isRequired,
};

export default withRouter(withStyles(styles)(EnhancedTableAgent4Rate));