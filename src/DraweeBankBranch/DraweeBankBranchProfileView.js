...
import DraweeBankBranchProfile from '../DraweeBankBranch/DraweeBankBranchProfile';
import AuditingComponent from  '../component/AuditingComponent';
import moment from 'moment';
import * as config from './../config/config';
import * as Exceptionhandler from '../ExceptionHandling';
import * as ApiService from './DraweeeBankBranchApiService';

class DraweeBankBranchProfileView extends Component{
    constructor(props){
        super(props)
        this.state={
            value:0,
    ...
    getActivity = () => {
        if(sessionStorage.getItem('token') == undefined){
          ...
          this.setState({ loading: true }, () => {
            ApiService.getActivity(this.props.match.params.draweeId, this.props.match.params.pId)
              .then((response) => {
  ...
  render(){    
    ...
    return(
        <EnchancedTabs value={this.state.value} getTabVal={this.handleTabVal} draweeLabel={"DRAWEE BANK PROFILES"} >
            {this.state.value === 0 && <DraweeBankBranchProfile draweeBankName={this.state.draweeBankName} draweeBankBranchProfileView={this.props.draweeBankBranchProfileView}/>}
...
export default DraweeBankBranchProfileView;