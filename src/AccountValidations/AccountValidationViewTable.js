            ...
            <Paper className={classes.root}>
                <div className={classes.tableWrapper}>
                    <Table className='global-font' style={{whiteSpace:`nowrap`}}>
                        ...
                        <TableBody>
                            {this.props.validationValue && this.props.validationValue.map(row=>{
                              return(
                                <TableRow >
                                    <TableCell style={{paddingLeft:40,fontSize:"14px"}}>{row.validationType}</TableCell>
                                    <TableCell style={{fontSize:"14px"}}>{row.position ? row.position : '--'}</TableCell>
                                    <TableCell style={{fontSize:"14px"}}>{row.range ? row.range : '--'}</TableCell>
                                    <TableCell style={{fontSize:"14px"}}>{row.value ? row.value : '--'}</TableCell>
                                </TableRow>
                              )                                  
                            })}                            
                        </TableBody>
